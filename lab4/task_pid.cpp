...
{	  
     ...
     // Creating the PID objects for Motor 1 and Motor 2
      ...
      int16_t Ki_2 = 0 * 256;					// K_i Integral gain
      int16_t Kw_2 = 0 * 1024;					// K_w Anti-windup gain
      int16_t min_2 = -max_motor_ticks;			// Minimum saturation limit
      int16_t max_2 = max_motor_ticks;			// Maximum saturation limit
	  ...
	  if (sh_PID_control->get() == 1)
	  {
	       ...
	       if(runs % 167 == 0)
	       {
		    //*p_serial << PMS ("setpoint_2_ticks: ") << setpoint_2_ticks << endl;
